...
- **2026-09-04: trigger superseded — P16 `[WON'T]`, P25 filed.** agent-sop P97 shipped the automation this phase was building, one level up: a user-scope Stop hook (`sop-stop-drift.sh`) that reads `ship-sop.config.json` and exits 2 with the gate demand in the same turn, plus a push gate (`sop-push-gate.sh`) that refuses an uncovered `git push` / `gh pr create`. Two harness facts made the Phase 3 plan's two-step design (Stop writes a directive, SessionStart picks it up) unfixable in place: project-scope hooks load only from the launch directory, and Stop stdout never reaches the model — so Batch 3.1's correctly-shaped entry still produced no live gate run on any consumer repo after 2026-08-03. The script itself fires when run by hand in a clone. Batches 3.3-3.10 that concern the hook's state machine (P18, P19 stamp-at-emission, staleness tautology, docs-only routing) are superseded by agent-sop's coverage-by-fact rule (a report names an ancestor of HEAD with zero code lines since); the remaining deterministic-checks half of P20 and the defects in P17, P22, P23 stand. Docs-only batch; wiring retirement is P25.

  **Reviewer turn (P14, P15):** `docs/reviews/2026-08-03_solo_P14-P15.md` — APPROVE, 0 CRITICAL / 0 HIGH / 1 MEDIUM / 2 LOW. Fired via Step 1b trigger (b), not tag: both items are `[Bug]` and would normally be exempt, but the session touched `.claude/commands/ship-on.md`. All three findings fixed in the same session. (1) `scripts/auto-ship-hook.sh:9-17` still documented the pre-P14 flat shape as what `setup.sh` writes — the one file P14 missed, and the one an operator would copy from to hand-wire a project. (2) The uninstall jq dropped a pre-existing `"hooks": []` entry that was never ours; now only an entry our own removal emptied is dropped. (3) The README manual-fallback jq had no existence guard, leaving a stray `.tmp` on a missing settings.json and adding an empty `.hooks.Stop` where none existed. Verified by round-trip: a settings.json carrying a user hook plus an empty-hooks entry comes back byte-equivalent after install then uninstall.
